...
class ProfileService:
    ...
    async def upsert(
        ...
        *,
        ...
        """Upsert a profile, optionally joining a caller-owned transaction."""
        if session is not None:
            return await self._upsert_in_session(session, update)
        async with self._session_factory() as owned_session:
            async with owned_session.begin():
                row = await self._upsert_in_session(owned_session, update)
    ...
    async def _upsert_in_session(
        ...
        if row is None:
            ...
            row.behavior_tags = sorted(set(row.behavior_tags or []).union(update.behavior_tags))
    ...
    async def apply_retention(self, limit: int = RISK_HISTORY_LIMIT) -> int:
        """Trim existing profiles to their most recent *limit* risk observations."""
        if limit < 1:
            raise ValueError("profile risk history limit must be positive")
        affected = 0
        async with self._session_factory() as session:
            async with session.begin():
                rows = list(
                    await session.scalars(
                        select(EntityProfileORM)
                        .order_by(EntityProfileORM.profile_id)
                        .with_for_update()
                    )
                )
                for row in rows:
                    ...
                    if len(history) <= limit:
                        continue
                    row.risk_history = history[-limit:]
                    row.updated_at = datetime.now(UTC)
                    affected += 1
        return affected
