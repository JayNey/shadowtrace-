...
class DecisionTraceSummary(BaseModel):
    ...
    active_duration_ms: int | None = Field(
        default=None,
        description=(
            "Effective investigation duration (ms): wall-clock span minus the "
            "union of (1) EventStatus WAITING_* halt gaps from STATE_TRANSITION "
            "to_status (primarily waiting_approval) and (2) ExecutionSubstate "
            "waiting_writeback gaps from EventContextJournal execution_substate "
            "changes (ISSUE-257). Overlapping gaps are merged once. "
