...
def diff_detection_artifacts(
    baseline: DetectionEvaluationArtifact,
    candidate: DetectionEvaluationArtifact,
) -> list[EvaluationGateDiff]:
    """Return machine-readable deltas between two detection evaluation artifacts."""
    ...
    baseline_refs = baseline.config.candidate_refs.model_dump()
    candidate_refs = candidate.config.candidate_refs.model_dump()
    if baseline_refs != candidate_refs:
        diffs.append(
            EvaluationGateDiff(
                field="config.candidate_refs",
                expected=baseline_refs,
                actual=candidate_refs,
                reason="candidate refs changed",
            )
        )
        ...
        if left is None or right is None:
            diffs.append(
                EvaluationGateDiff(
                    ...
                    expected="present",
                    actual="missing",
                    reason=f"case {case_id} missing from one artifact",
                )
            )
            continue
...
def diff_detection_against_baseline(
    baseline: DetectionEvaluationArtifact,
    candidate: DetectionEvaluationArtifact,
) -> list[EvaluationGateDiff]:
    """Compare candidate output to a pinned baseline detection artifact."""
    ...
    aligned = finalize_detection_artifact(aligned)
    return diff_detection_artifacts(baseline, aligned)


__all__ = ["diff_detection_against_baseline", "diff_detection_artifacts"]
