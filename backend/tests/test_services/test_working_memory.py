...
async def test_owner_write_success_and_access_log(
    ...
    value = await wm.read(event_id, "triage_result", reader="RiskAgent")
...
async def test_non_owner_write_rejected_and_logged(
    ...
    event_id = await _seed_event(session_factory)
    await store.init_context(event_id, _summary(event_id))
    await wm.write(event_id, "triage_result", {"ok": True}, writer="TriageAgent")
...
async def test_false_positive_hook_alias_allowed(
    ...
    await wm.write(
        ...
        {"close_as_fp": True},
        writer="RuleBasedFalsePositiveHook",
    )
    assert await store.get(event_id, "false_positive_match") == {"close_as_fp": True}


@pytest.mark.asyncio
async def test_wm_strict_false_allows_non_owner_with_warning(
    store: EventContextStore,
    redis_client: RedisClient,
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    ...
    assert await store.get(event_id, "triage_result") == {"via": "non-owner"}
...
async def test_version_conflict_retries_then_succeeds(
    ...
    event_id = await _seed_event(session_factory)
    await store.init_context(event_id, _summary(event_id))
    await wm.write(event_id, "graph_output", {"nodes": 1}, writer="GraphAgent")
    ...
    with patch.object(store, "compare_and_set", side_effect=flaky_cas):
        await wm.write(event_id, "graph_output", {"nodes": 2}, writer="GraphAgent")
...
async def test_stale_redis_version_does_not_block_owner_write(
    ...
    event_id = await _seed_event(session_factory)
    await store.init_context(event_id, _summary(event_id))
    await wm.write(event_id, "graph_output", {"n": 1}, writer="GraphAgent")
    ...
    await wm.write(event_id, "graph_output", {"n": 2}, writer="GraphAgent")
...
async def test_scratchpad_append_and_fifo_roll(
    ...
    for i in range(SCRATCHPAD_LIMIT + 5):
        await wm.append_scratchpad(event_id, "TriageAgent", f"note-{i}")

    entries = await wm.read_scratchpad(event_id)
