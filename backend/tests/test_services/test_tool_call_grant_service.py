...
async def test_reserve_attempt_counts_denied_paths(service: ToolCallGrantService) -> None:
    ...
    first = await service.reserve_attempt(
        grant,
        tool_name="query_dns",
        params={"domain": "example.com"},
        event_id=event_id,
    )
    second = await service.reserve_attempt(
        grant,
        tool_name="query_dns",
...
async def test_expired_grant_denied(service: ToolCallGrantService) -> None:
    ...
    issued = await service.issue_grant(request)
    grant = issued.grant
    grant.expires_at = datetime.now(tz=UTC) - timedelta(seconds=1)
...
async def test_concurrent_reserve_respects_max_calls(service: ToolCallGrantService) -> None:
    ...
    async def _one() -> int | None:
        try:
            attempt = await service.reserve_attempt(
                grant,
                tool_name="query_dns",
    ...
    assert await service.count_attempts(grant.grant_id) == 5
